// Wearable provider brand marks, rendered as inline SVG.
//
// Strava + Garmin use the official brand glyphs (paths from the simple-icons
// open registry of brand logos), rendered in each brand's official color.
//
// WHOOP, Polar, Suunto, and Ultrahuman are NOT in simple-icons. Until their
// official SVGs are supplied, they use recognizable brand-colored placeholders.
// Add an entry to OFFICIAL_PATHS to upgrade any of them.
...
// Official brand glyphs (24x24 viewBox), single-path, from simple-icons.
const OFFICIAL_PATHS: Partial<Record<OWProvider, { path: string; color: string }>> = {
  strava: {
    color: '#FC4C02',
  ...
  garmin: {
    // Official Garmin wordmark glyph — rendered white so it reads on dark cards.
    color: '#FFFFFF',
...
export function ProviderLogo({ provider, size = 26 }: ProviderLogoProps) {
  const official = OFFICIAL_PATHS[provider]
  if (official) {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-label={provider}>
        <path d={official.path} fill={official.color} />
      </svg>
    )
  }

  // Placeholder brand-colored marks — pending official SVGs for these four.
