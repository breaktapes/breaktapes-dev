      ...
      <div style={st.greetingContent}>
        ...
        {geoState === 'ok' && weather ? (
                ...
                {weather.hourly.map((h, i) => (
                  <div key={i} style={{
                    ...
                    {h.temp !== null
                      ...
                      : <span style={{ fontFamily: 'var(--headline)', fontWeight: 700, fontSize: 'var(--text-xs)', color: 'var(--orange)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{h.isSun === 'rise' ? 'RISE' : 'SET'}</span>
                    }
                    <span style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', letterSpacing: '0.02em', textAlign: 'center', whiteSpace: 'nowrap' }}>{h.time}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <button
            ...
            style={{ marginTop: '8px', background: 'rgba(var(--orange-ch),0.12)', border: '1px solid rgba(var(--orange-ch),0.35)', borderRadius: 'var(--radius-md)', padding: '8px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', color: 'var(--orange)', fontFamily: 'var(--headline)', fontWeight: 700, fontSize: 'var(--text-xs)', letterSpacing: '0.06em', textTransform: 'uppercase' }}
          >
            Tap to show local weather
          </button>
        ) : geoState === 'loading' ? (
          <div style={{ marginTop: '8px', fontSize: 'var(--text-xs)', color: 'var(--muted)' }}>Getting your weather…</div>
        ) : geoState === 'denied' ? (
    ...
    <div style={{ ...st.briefingCard, border: '1px solid rgba(232,78,27,0.4)' }}>
      ...
      <div style={st.briefingInner}>
        ...
        {goalPace ? (
          ...
          <div style={{ marginTop: '10px', fontSize: 'var(--text-xs)', color: 'var(--muted)', fontFamily: 'var(--body)' }}>
            No goal time set —{' '}
            <span
        ...
        {/* Gear checklist */}
        <div style={{ marginTop: '10px' }}>
          <div style={{ fontFamily: 'var(--headline)', fontWeight: 900, fontSize: 'var(--text-xs)', letterSpacing: '0.12em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '6px' }}>Gear Checklist</div>
          ...
          <div style={{ display: 'flex', gap: 'var(--sp-2)', marginTop: '8px' }}>
            <input
              ...
              style={{
                ...
                fontFamily: 'var(--body)', fontSize: 'var(--text-xs)', color: 'var(--white)',
                outline: 'none',
              }}
            />
            <button
              ...
              style={{
                ...
                fontWeight: 800, fontSize: 'var(--text-xs)', letterSpacing: '0.08em',
                textTransform: 'uppercase', cursor: 'pointer',
              }}
            >
              Add
            ...
            <div style={{ display: 'flex', gap: 'var(--sp-2)' }}>
              {PRIORITIES.map(p => (
                ...
                >
                  ...
                  <span style={{ fontSize: 'var(--text-xs)', fontFamily: 'var(--headline)', fontWeight: 700, letterSpacing: '0.06em', color: priority === p.key ? 'var(--orange)' : 'var(--muted)', textTransform: 'uppercase' }}>{p.label}</span>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', lineHeight: 1.3 }}>{p.desc}</span>
                </button>
              ))}
            </div>
          </div>
...
function CountdownCard({ race, onShowAll, upcomingRaces, onSelectRace }: { race: Race; onShowAll: () => void; upcomingRaces: Race[]; onSelectRace: (id: string) => void }) {
  ...
  return (
    <>
      ...
      <WidgetCard id="countdown" style={st.countdownCard} hint={false} noDetailPreview>
        {/* Header row */}
              ...
              {qualRaces.slice(0, 3).map(r => {
                ...
                return (
                  <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', background: 'var(--surface)', borderRadius: 'var(--radius-sm)', gap: 'var(--sp-2)' }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--white)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
  ...
  return (
    <WidgetCard id="career-momentum" style={st.glowCard}>
      ...
      <div>
        ...
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', marginTop: '2px' }}>→ vs last block</div>
...
function RaceDNAWidget() {
  ...
  const dna = useMemo(() => {
    ...
    type TempBucket = { label: string; short: string; color: string; races: Race[]; avgPace: number | null }
    const rawBuckets = [
      { label: 'Cold',  short: '<10°', color: '#60A5FA', races: withTemp.filter(r => r.weather!.temp! <  10) },
      { label: 'Cool',  short: '10–20°', color: '#34D399', races: withTemp.filter(r => r.weather!.temp! >= 10 && r.weather!.temp! < 20) },
      { label: 'Warm',  short: '20–28°', color: '#FBBF24', races: withTemp.filter(r => r.weather!.temp! >= 20 && r.weather!.temp! < 28) },
      { label: 'Hot',   short: '28°+',  color: '#F87171', races: withTemp.filter(r => r.weather!.temp! >= 28) },
    ]
    const bucketWithPace: TempBucket[] = rawBuckets.map(b => {
      const paces = b.races
        .map(r => computePaceSecPerKmFn(r.distance, r.time!))
        .filter((p): p is number => p != null)
      const avgPace = paces.length ? paces.reduce((a, x) => a + x, 0) / paces.length : null
      return { ...b, avgPace, count: b.races.length } as TempBucket & { count: number }
    }).filter(b => b.races.length > 0)

    const bestTempBucket = bucketWithPace.filter(b => b.avgPace != null).length
      ? bucketWithPace.filter(b => b.avgPace != null).reduce((a, b) => (b.avgPace! < a.avgPace! ? b : a))
      : null
    // Compute pace delta % relative to best
    const bestPace = bestTempBucket?.avgPace ?? null
    const tempBarsData = bucketWithPace.map(b => ({
      ...b,
      delta: bestPace && b.avgPace ? Math.round(((b.avgPace - bestPace) / bestPace) * 100) : 0,
      isBest: bestTempBucket?.label === b.label,
    }))
    ...
    const surfaces = Object.entries(surfMap)
      ...
      .map(([s, n]) => ({
        ...
        count: n,
        pct: Math.round((n / past.length) * 100),
      }))
    ...
    for (const r of withSplits) {
      ...
      if (avgSecond < avgFirst * 0.97) negSplit++
    }
    const even = withSplits.length - faded - negSplit
    const pacingPersona = withSplits.length === 0 ? null
      : faded > negSplit && faded > even ? 'FADER'
      : negSplit > faded && negSplit > even ? 'NEGATIVE SPLITTER'
      : 'EVEN PACER'
    const pacingPersonaColor = pacingPersona === 'NEGATIVE SPLITTER' ? '#34D399'
      : pacingPersona === 'FADER' ? '#F87171'
      : 'var(--orange)'
    const pacingPersonaDesc = pacingPersona === 'FADER'
      ? `Go out too fast in ${faded} of ${withSplits.length} races`
      : pacingPersona === 'NEGATIVE SPLITTER'
      ? `Finish strong in ${negSplit} of ${withSplits.length} races`
      : `Consistent pacing in ${even} of ${withSplits.length} races`
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-1)', flexShrink: 0 }}>
                  {[
                    ...
                    <div key={x.label} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      ...
                      <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', width: '26px' }}>{x.label}</div>
                      <div style={{ fontSize: 'var(--text-xs)', color: x.color, fontWeight: 700 }}>{x.count}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--sp-2)' }}>
                {dna.countryList.map(([country, count]) => (
                  <div key={country} style={{
                    ...
                    <span>{country}</span>
                    <span style={{ color: 'var(--muted)', fontSize: 'var(--text-xs)' }}>{count}</span>
                  </div>
                ))}
                {dna.travelCount > 5 && (
...
function PatternScanWidget() {
  ...
  const scan = useMemo(() => {
    ...
    const seasonPace: { season: string; emoji: string; avg: number; count: number }[] = []
    for (const [season, months] of Object.entries(SEASON)) {
      const paces = months.flatMap(m => monthPerf[m] ?? [])
      if (paces.length) seasonPace.push({ season, emoji: SEASON_EMOJI[season], avg: paces.reduce((a,b) => a+b,0)/paces.length, count: paces.length })
    }
    ...
    // Compute normalized bar widths
    const maxSeasonPace = seasonPace.length ? Math.max(...seasonPace.map(s => s.avg)) : 1
    const minSeasonPace = seasonPace.length ? Math.min(...seasonPace.map(s => s.avg)) : 1
    const seasonRange = maxSeasonPace - minSeasonPace || 1
    const seasonBars = seasonPace.map(s => ({
      ...s,
      barPct: Math.round(100 - ((s.avg - minSeasonPace) / seasonRange) * 60), // best = 100%
      isBest: bestSeason?.season === s.season,
    }))
    ...
    const topDist = Object.entries(distMap).sort((a,b) => b[1]-a[1]).slice(0,4)
    const maxDistCount = topDist[0]?.[1] ?? 1
          ...
          {scan.seasonBars.length > 0 && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                {scan.bestSeason && (
                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--orange)', fontWeight: 700 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
                {(ptSize === 'medium' ? scan.seasonBars.filter(s => s.isBest) : scan.seasonBars).map(s => (
                  <div key={s.season} style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)' }}>
                    ...
                    <div style={{ width: '20px', fontSize: 'var(--text-xs)', color: 'var(--muted)', textAlign: 'right', flexShrink: 0 }}>{s.count}</div>
                  </div>
                ))}
              </div>
            </div>
    ...
    <WidgetCard id="personal-bests" style={st.glowCard} noDetailPreview>
      ...
      {showEdit && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)', marginTop: '10px', padding: '12px', background: 'var(--surface3)', borderRadius: 'var(--radius-md)' }}>
          {pillGroups.map(pg => (
            <div key={pg.sport}>
              <div style={{ fontSize: 'var(--text-xs)', fontFamily: 'var(--headline)', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--muted2)', marginBottom: '8px' }}>
                {pg.sport}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
                {pg.labels.map(label => {
      ...
      {expired.map(race => (
        <div key={race.id} style={{
          ...
          <div style={{ display: 'flex', gap: 'var(--sp-2)', flexShrink: 0 }}>
            <button
              onClick={() => onLogResult()}
              style={{ background: 'var(--orange)', color: '#000', border: 'none', borderRadius: 'var(--radius-md)', padding: '8px 14px', fontFamily: 'var(--headline)', fontWeight: 900, fontSize: 'var(--text-xs)', letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer' }}
            >
              LOG
            </button>
            <button
              onClick={() => dismissExpired(race.id)}
              style={{ background: 'transparent', color: 'var(--muted)', border: '1px solid var(--border2)', borderRadius: 'var(--radius-md)', padding: '8px 10px', fontFamily: 'var(--headline)', fontWeight: 700, fontSize: 'var(--text-xs)', letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer' }}
            >
              SKIP
            </button>
          </div>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 'var(--text-sm)', color: row.isSameAsInput ? 'var(--orange)' : 'var(--white)', fontWeight: row.isSameAsInput ? 700 : 400 }}>
                    {row.distance}
                    {row.isSameAsInput && <span style={{ fontSize: 'var(--text-xs)', marginLeft: '6px', color: 'var(--muted)', textTransform: 'uppercase', fontFamily: 'var(--headline)' }}>actual</span>}
                  </span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)' }}>
                    ...
                    {hasGoal && (
                      <span style={{ fontSize: 'var(--text-xs)', color: 'var(--green)', fontFamily: 'var(--headline)', fontWeight: 700 }}>✓ GOAL SET</span>
                    )}
                  </div>
                </div>
                {matched.length > 0 && (
                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function GoalPaceWidget({ race }: { race: Race | null }) {
  ...
  return (
    <WidgetCard id="goal-pace" style={st.glowCard}>
      ...
      {/* VDOT gap */}
      {gapLabel && (
        <div style={{ fontSize: 'var(--text-xs)', color: gapColor, padding: '6px 10px', background: 'var(--surface3)', borderRadius: 'var(--radius-sm)', border: `1px solid ${gapColor}33` }}>
          {gapLabel}
                  ...
                  return (
                    <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: '1px solid var(--border)' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted2)' }}>{fmtDateDDMM(r.date)}</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)' }}>
...
const st = {
  ...
  glowCard: {
    ...
    borderRadius: '16px',
    padding: 'var(--sp-4)',
    boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.03), 0 12px 28px rgba(0,0,0,0.18)',
    display: 'flex',
    flexDirection: 'column' as const,
    gap: 'var(--sp-4)',
    minWidth: 0,
